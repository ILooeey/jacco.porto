...
if(isset($_GET['id'])) {
    ...
    foreach($data as $d){
        $judul = $d['Judul_Artikel'];
        $tanggal = $d['Tanggal_Artikel'];
        $deskripsi = $d['Deskripsi_Artikel'];
        $jalan = $d['Jalan_Artikel'];
        $id_desa = $d['id_Desa'];
...
if(isset($_POST['submit_tambah'])){
  ...
  $query = "UPDATE INTO data_artikel (Judul_Artikel, Tanggal_Artikel, Deskripsi_Artikel, Jalan_Artikel, id_Desa, id_Kecamatan) 
        VALUES (?, ?, ?, ?, ?, ?)";
    ...
    <div class="container mt-5">
      ...
      <form action="edit_artikel.php" method="post">
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="judulArtikel" name="judulArtikel" value="<?php echo $judul; ?>" required>
        </div>
        <div class="mb-3">
          <label for="tanggalArtikel" class="form-label">Tanggal Artikel</label>
          <input type="date" class="form-control" id="tanggalArtikel" name="tanggalArtikel" value="<?php echo $tanggal; ?>" required>
        </div>
        <div class="mb-3">
          <label for="deskripsiArtikel" class="form-label">Deskripsi Artikel</label>
          <textarea class="form-control" id="deskripsiArtikel" name="deskripsiArtikel" rows="3" required><?php echo $deskripsi; ?></textarea>
        </div>
        <div class="mb-3">
        ...
        <form action="edit_artikel.php" method="post" style="display: inline-block;">
            <input type="hidden" name="id_submit" value="id_Artikel">
            <button type="submit" class="btn btn-primary float-end" name="submit_tambah">Simpan</button>
        </form>
      </form>
    </div>
